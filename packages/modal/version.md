...
## version 0.6.8
- 모달안에 여러 input이 있을때 한글을 입력 후 tab으로 포커스 이동하면 해당 내용이 그대로 복사가 된 후 바로 다음 포커스로 넘어가는 문제가 있었음.
- 사파리에서는 해당 문제가 발생하지 않고 크롬에서만 발생했는데 찾아보니 IME(입력기) 문제라고 함.
- 정확한 원인은 찾지 못했지만, compostionStart와 compositionEnd 이벤트를 이용하여 해당 문제를 해결함
...
## version 0.7.2
- 모달 포커스 핸들링을 option을 통해서 비활성화할 수 있도록 기능을 추가함.